...
const VacancyDetailsDialog: React.FC<VacancyDetailsDialogProps> = ({ 
  ...
  const handleAcceptVacancy = async () => {
    ...
    try {
      ...
      if (response.ok) {
        alert('Вакансия успешно принята!');
        onClose();
        ...
        alert(data.detail || 'Не удалось принять вакансию');
      }
    } catch (error) {
      console.error('Error accepting vacancy:', error);
      alert('Произошла ошибка при принятии вакансии');
    } finally {
  ...
  const handleCloseVacancy = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(error.detail || 'Не удалось закрыть вакансию');
      }
    } catch (error) {
      console.error('Error closing vacancy:', error);
      alert('Произошла ошибка при закрытии вакансии');
    } finally {
      setButtonLoading(false);
    }
  };
