...
export default function AnalyticsPage() {
  ...
  const getMonthFilterParams = () => {
    ...
    console.log('Период фильтрации:', { 
      месяц: selectedMonth, 
      год: currentYear,
      начало: startDate.toLocaleString('ru-RU'),
      конец: endDate.toLocaleString('ru-RU'),
      начало_iso: startDate.toISOString(),
      конец_iso: endDate.toISOString()
    });
        ...
        <DialogContent className="bg-[#0a1929] border border-[#1e3a5f]/50 shadow-lg shadow-blue-900/20 text-white max-w-5xl">
          <DialogHeader>
            <DialogTitle className="flex justify-between items-center">
              ...
              {/* Удаляем кнопки "По месяцам" и "По дням", так как эта функциональность уже есть в самом компоненте */}
            </DialogTitle>
          </DialogHeader>
          ...
          <div className="mt-4">
            <StatusHistoryChart
              ...
              displayMode={statusHistoryDisplayMode}
            />
          </div>
        </DialogContent>
      </Dialog>
