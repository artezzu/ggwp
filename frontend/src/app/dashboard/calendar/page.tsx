...
export default function CalendarPage() {
  ...
  const fetchTasks = async () => {
    try {
      ...
      const tasksWithDefaults = data.map((task: any) => {
        // Обрабатываем время, если его нет
        const taskWithTime = {
          ...task,
          time: task.time || '00:00' // Добавляем время по умолчанию, если его нет
        };
        
        // Приоритизируем обработку информации о создателе
        // 1. Сначала используем поле creator если оно есть
        if (task.creator) {
          console.log('Задача с полем creator:', task.creator);
          taskWithTime.created_by = {
            id: task.creator.id,
            name: task.creator.full_name, // Явно используем full_name
            email: task.creator.email,
            role: task.creator.role
          };
        } 
        // 2. Если есть created_by_id и это текущий пользователь
        else if (task.created_by_id && currentUser && task.created_by_id === currentUser.id) {
          console.log('Задача создана текущим пользователем:', currentUser);
          taskWithTime.created_by = {
            id: currentUser.id,
        ...
        // 3. Если есть только created_by в виде строки (email)
        else if (typeof task.created_by === 'string') {
          console.log('Задача с email вместо объекта created_by:', task.created_by);
          
          // Если это email текущего пользователя, используем его данные
          if (currentUser && currentUser.email === task.created_by) {
            ...
            // Иначе создаем объект с базовой информацией
            taskWithTime.created_by = {
              id: 0,
              name: `Пользователь (${task.created_by.split('@')[0]})`, // Более понятное имя
              email: task.created_by
            };
          }
        }
        // 4. Если created_by это объект, но в нем нет имени или имя равно email
        else if (task.created_by && typeof task.created_by === 'object') {
          console.log('Задача с объектом created_by:', task.created_by);
          
          if (!task.created_by.name || task.created_by.name === task.created_by.email) {
            // Если это email текущего пользователя
            if (currentUser && task.created_by.email && currentUser.email === task.created_by.email) {
              taskWithTime.created_by = {
                ...task.created_by,
                id: currentUser.id,
                name: currentUser.name,
            ...
            } else if (task.created_by.email) {
              // Создаем более читаемое имя из email
              const emailName = task.created_by.email.split('@')[0];
              taskWithTime.created_by = {
                ...task.created_by,
                name: `Пользователь (${emailName})` // Более понятное имя
              };
            }
          }
        }
        // Если никаких данных о создателе нет и это не демо-режим
        else if (!taskWithTime.created_by && currentUser) {
          console.log('Нет данных о создателе, используем текущего пользователя:', currentUser);
          taskWithTime.created_by = {
            id: currentUser.id,
            name: currentUser.name,
            email: currentUser.email,
  ...
  useEffect(() => {
    if (creatorFilter === null) {
      ...
      setFilteredTasks(tasks.filter(task => 
        task.created_by && task.created_by.id === creatorFilter
      ));
    }
  }, [tasks, creatorFilter]);
  ...
  const openTaskDialog = (task: Task) => {
    ...
    setNewTask({
      ...
      date: task.date,
      time: task.time || '00:00', // Используем существующее время или устанавливаем значение по умолчанию
      status: task.status
    });
              ...
              {week.map((day, j) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-1 space-y-0.5 overflow-y-auto max-h-[60px] scrollbar-thin">
                      {dayTasks.map((task) => {
