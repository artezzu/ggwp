...
def get_tasks(
    ...
    # Если пользователь не администратор, то возвращаем только его задачи
...
def get_task(task_id: int, current_user: schemas.User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if db_task.creator:
        print(f"  Данные создателя: {db_task.creator.full_name} ({db_task.creator.email})")
...
def update_task(
    ...
    # Проверяем, что пользователь имеет доступ к задаче
    if current_user.role != "admin" and db_task.assigned_to != current_user.id and db_task.created_by_id != current_user.id:
        raise HTTPException(status_code=403, detail="Нет доступа к этой задаче")
